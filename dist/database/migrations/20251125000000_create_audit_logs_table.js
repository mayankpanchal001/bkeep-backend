...
async function up(knex) {
    await knex.schema.createTable("audit_logs", (table) => {
        ...
        table
            .string("action", 100)
            .notNullable()
            .comment("Namespaced action (e.g., user.created, user.updated, user.deleted, user.logged_in)");
        ...
        table
            .jsonb("context")
            .notNullable()
            .comment("Request metadata: { ip, userAgent, method, endpoint, requestId }");
        // Success status
        table
            .boolean("success")
            .notNullable()
    ...
    await knex.raw("CREATE INDEX idx_audit_logs_actor_gin ON audit_logs USING GIN (actor jsonb_path_ops)");
    await knex.raw("CREATE INDEX idx_audit_logs_targets_gin ON audit_logs USING GIN (targets jsonb_path_ops)");
    await knex.raw("CREATE INDEX idx_audit_logs_context_gin ON audit_logs USING GIN (context jsonb_path_ops)");
}
